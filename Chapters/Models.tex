...
James M. Sakoda's model, initially conceptualized in his 1949 dissertation and fully introduced in ``The Checkerboard Model of Social Interaction,'' pre-dates Schelling's work and offers a more nuanced approach to modeling social interactions using a similar checkerboard framework. Unlike Schelling's focus solely on segregation dynamics, Sakoda's model incorporates a broader range of social interactions by allowing agents to exhibit positive, neutral, or negative attitudes towards their neighbors. These attitudes influence the agents' movements across the board, aiming to optimize their local environment according to specific utility functions that aggregate the effects of surrounding agents. Sakoda's model is capable of simulating a variety of social phenomena beyond segregation, such as the formation of stable social clusters and the dynamics of group interactions.

Both models employ a checkerboard as the computational space where agents (or tokens) reside and interact according to predefined rules. In Schelling's model, the primary technical aspect is the agents' decision-making process based on neighborhood composition. Agents assess the racial makeup within a defined local neighborhood (typically the eight surrounding cells) and decide to move if the proportion of similar agents falls below a specified threshold. This model is particularly noted for its application of simple rules that lead to complex emergent behaviors, effectively demonstrating how micro-level preferences translate into macro-level patterns.

In contrast, Sakoda's model is distinguished by its incorporation of a wider range of interactions through the use of an attitude matrix that assigns numerical values to represent different types of interactions between agents. This matrix allows for a more detailed specification of social forces at play, including attraction, repulsion, and indifference. The computation in Sakoda's model involves evaluating the total utility for an agent based on the sum of influences from all other agents on the board, weighted by distance. This approach allows for the exploration of more complex dynamics and the potential for more varied and realistic outcomes.

Both models have profoundly influenced the field of computational social science, providing valuable insights into the dynamics of social systems. They highlight the importance of local interactions in shaping global patterns and have applications in areas ranging from urban planning to the study of social networks.
